...
public class Communicator {
    ...
    public static boolean shouldSniff(MapLocation loc) throws GameActionException {
        for(int i = 41; i < 52; i++) {
            if(getLoc(rc.readSharedArray(i)).isWithinDistanceSquared(loc, 2)) {
                return false;
            }
        }
        return true;
    ...
    public static void rememberFlags(RobotController rc) throws GameActionException {
        for(int i = 1; i < 4; i++) {
            ...
            myFlagIds[i - 1] = rc.readSharedArray(i+3);
        }
    }
    static MapLocation[] myFlags = new MapLocation[3];
    static int[] myFlagIds = new int[3];
    ...
    public static void droppedFlagAt(RobotController rc)throws GameActionException  {
        ...
        for(int i = 1; i < 64; i++) {
            if(rc.readSharedArray(i) == 0) {
                ...
                rc.writeSharedArray(i+3, rc.senseNearbyFlags(0,rc.getTeam())[0].getID());
                myFlagId = i;
                break;
            }
        }
    ...
    public static void reportHotZone(MapLocation loc)  throws GameActionException{
        for(int i = 41; i-->33; ) {
            if(rc.readSharedArray(i) == 0) continue;
            if(getLoc(rc.readSharedArray(i)).isWithinDistanceSquared(loc,16)) return;
        }
        for(int i = 41; i-->33; ) {
            if(rc.readSharedArray(i) == 0) {
                rc.writeSharedArray(i, getInt(loc) | (5 << 12));
                break;
            }
        }
    }

    public static void unreportHotZone(MapLocation loc) throws GameActionException {
        for(int i = 41; i-->33; ) {
            if(rc.readSharedArray(i) == 0) continue;
            if(getLoc(rc.readSharedArray(i)).isWithinDistanceSquared(loc, 4)) {
                rc.writeSharedArray(i, 0);
                break;
            }
        }
    ...
    public static MapLocation closestHotZone(MapLocation loc) throws GameActionException {
        int minDist = 10000000;
        MapLocation closest = null;
        for(int i = 41; i-->33; ) {
            if(rc.readSharedArray(i) == 0) continue;
            if(getLoc(rc.readSharedArray(i)).distanceSquaredTo(loc) < minDist) {
                minDist = getLoc(rc.readSharedArray(i)).distanceSquaredTo(loc);
                closest = getLoc(rc.readSharedArray(i));
            }
        }
        return closest;
    }
