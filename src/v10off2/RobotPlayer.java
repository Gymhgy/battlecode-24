...
public strictfp class RobotPlayer {
    ...
    public static void spawn(RobotController rc) throws GameActionException {
        ...
        for(MapLocation spawnLoc : spawnLocs) {
            ...
            MapLocation hz = Communicator.closestHotZone(spawnLoc);
            if(task != null) {
                if (spawnLoc.distanceSquaredTo(hz) < minDist) {
                    minDist = spawnLoc.distanceSquaredTo(hz);
                    spawn = spawnLoc;
                }
            }
        }*/
    ...
    public static void play(RobotController rc) throws GameActionException {
        ...
        checkNearbyFlag(rc);
        if(myFlag != null) {
            Communicator.reportCarrying((int)Math.min(Math.max(0, 5 - friendCnt + enemyCnt), 15));
        }
        updateStuns(rc);
        recordStuns(rc);
/*
        if(enemyCnt > 5 && friendCnt > 4)
            Communicator.reportHotZone(rc.getLocation());
        else if (enemyCnt < 5) {
            Communicator.unreportHotZone(rc.getLocation());
        }*/
    }
