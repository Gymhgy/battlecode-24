...
public class Communicator {
    ...
    public static void reportStuck(MapLocation loc, int fid) throws GameActionException {
        int idx = fid + 32;
        int e = rc.readSharedArray(idx);
        int turns = e >> 12;
        if(getLoc(e & 0b1111_1111_1111).isWithinDistanceSquared(loc, 2)) {
            turns ++;
        }
        else {
            turns = 0;
        }
        turns = Math.min(turns, 15);
        e = (e & 0b1111_1111_1111) | (turns << 12);
        rc.writeSharedArray(idx, e);
    }
    public static boolean isStuck(int i) throws GameActionException {
        int idx = i+32;
        int e = rc.readSharedArray(idx);
        return e >> 12 > 9;
    }
