...
public class MicroAttacker {
    ...
    static boolean anyInRange = false;
    ...
    boolean doMicro(ArrayList<StunInfo> stuns, MapLocation target){
        try {
            ...
            if (bestMicro.dir == Direction.CENTER && bestMicro.minDistanceToEnemy <= 4 && !anyInRange) return true;

            if (rc.canMove(bestMicro.dir)) {
                rc.move(bestMicro.dir);
                minDist = bestMicro.minDistanceToEnemy;
                return true;
            }
    ...
    class MicroInfo {
        ...
        void updateEnemy(RobotInfo unit){
            ...
            if (dist < minDistanceToEnemy)  {
                ...
                closestEnemy = unit.getLocation();
            }
            if (dist <= currentActionRadius) {
                DPSreceived += currentDPS;
                anyInRange = true;
            }
        ...
        void updateAlly(RobotInfo unit){
            if (!canMove) return;
            if(unit.getLocation().isWithinDistanceSquared(closestEnemy, 10))
        ...
        boolean isBetter(MicroInfo M){
           ...
           /* if(DPSreceived >= hp && M.DPSreceived < hp) return false;
            if(DPSreceived < hp && M.DPSreceived > hp) return true;*/
