        ...
        while(rc.isActionReady()) {
            ...
            if(builder && macroMoved) {
                if (friendCnt + enemyCnt > 15 ||
                        ...
                        (rc.getCrumbs() > 3000) && friendCnt + enemyCnt*1.5 > 10) {
                    for (Direction d : dirs) {
    ...
    static void getTask(RobotController rc, MapLocation fromLoc) throws GameActionException {
        ...
        if(task != null) {
            if (flagLoc == null && approxFlagLog != null) {
                if(fromLoc.distanceSquaredTo(task) > fromLoc.distanceSquaredTo(approxFlagLog)) {
                    task = null;
                }
            }
            else if (flagLoc != null && fromLoc.distanceSquaredTo(task) * 1.5 > fromLoc.distanceSquaredTo(flagLoc)) {
                task = null;
            }
        }
    }
